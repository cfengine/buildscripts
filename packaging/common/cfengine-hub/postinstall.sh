...
if [ ! -f $CFENGINE_MP_CERT ]; then
  ...
  ${CFENGINE_OPENSSL} genrsa -passout pass:x -out ${CFENGINE_MP_PASS_KEY} 2048
  ...
  ${CFENGINE_OPENSSL} req -new -subj "/CN=$CFENGINE_LOCALHOST" -key ${CFENGINE_MP_KEY} -out ${CFENGINE_MP_CSR}

  # Generate CRT
  ${CFENGINE_OPENSSL} x509 -req -days 3650 -in ${CFENGINE_MP_CSR} -signkey ${CFENGINE_MP_KEY} -out ${CFENGINE_MP_CERT}
