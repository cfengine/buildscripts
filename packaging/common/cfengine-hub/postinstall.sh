...
CFENGINE_MP_DEFAULT_SSLCONF_LOCATION="$PREFIX/httpd/ssl/conf"
...
CFENGINE_MP_SSLCONF=$CFENGINE_MP_DEFAULT_SSLCONF_LOCATION/$CFENGINE_LOCALHOST.conf
...
if [ ! -f $CFENGINE_MP_CERT ]; then
  ...
  # Generate CRT
  ${CFENGINE_OPENSSL} x509 -req -days ${CFENGINE_SSL_DAYS_VALID} -in ${CFENGINE_MP_CSR} -signkey ${CFENGINE_MP_KEY} -out ${CFENGINE_MP_CERT} -extensions SAN -extfile "$CFENGINE_MP_SSLCONF"
